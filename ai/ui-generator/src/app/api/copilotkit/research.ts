...
async function curate(state: {
    ...
    const response = await model().invoke(
        [
            new SystemMessage(`You are a personal newspaper editor. 
         ...
         .`.replace(/\s+/g, ' ')),
            new HumanMessage(`Today's date is ${new Date().toLocaleDateString('en-GB')}.
...
async function critique(state: {
    ...
    const response = await model().invoke([
        new SystemMessage(`You are a personal newspaper writing critique. Your sole purpose is to provide short feedback on a written 
      ...
      Please return a string of your critique or [DONE].`.replace(/\s+/g, ' ')),
        new HumanMessage(`${feedbackInstructions}
...
async function write(state: {
    ...
    const response = await model().invoke([
        new SystemMessage(`You are a personal newspaper writer. Your sole purpose is to write a well-written article about a 
      topic using a list of articles. Write 5 paragraphs in markdown.`.replace(
                /\s+/g,
                ' '
            )),
        new HumanMessage(`Today's date is ${new Date().toLocaleDateString('en-US')}.
...
async function revise(state: {
    ...
    const response = await model().invoke([
        new SystemMessage(`You are a personal newspaper editor. Your sole purpose is to edit a well-written article about a 
      topic based on given critique.`.replace(/\s+/g, ' ')),
        new HumanMessage(`Your task is to edit the article based on the critique given.
      This is the article: ${state.agentState.article}
...
export const researchWithLangGraph = async (topic: string) => {
    const inputs = {
        agentState: {
            topic
        }
    ...
    return result.agentState.article.replace(regex, '');
};